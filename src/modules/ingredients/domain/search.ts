 ...
 *
 * Spanish and Catalan ingredient names are full of accents and nobody
...
/**
 * Decides whether an ingredient matches a search term.
 *
 * Matching runs against every language the ingredient has, not only the
 * one being displayed: the catalog is only partly translated, so someone
 * reading it in Catalan should still find an ingredient by typing its
 * Spanish name.
 *
 * The `code` is matched too. It costs nothing, and it is already the
 * unaccented ASCII spelling of the ingredient, so it also catches anyone
 * pasting a slug straight in.
 * @param term The raw search term as typed.
 * @param code The ingredient's stable slug.
 * @param names Every name the ingredient has, in any language.
 * @returns True when the ingredient should appear in the results.
 */
